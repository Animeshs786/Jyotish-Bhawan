...
const GroupConsultationTransaction = require("../../../models/groupConsultationTransaction");

moment.tz.setDefault("Asia/Kolkata");

// Create a new consultation transaction
exports.createGroupConsultationTransaction = catchAsync(
  async (req, res, next) => {
    const { consultationPackage, type, amount, gstAmount, customerDetail } =
      req.body;
    ...
    if (!type || !["chat", "call", "video"].includes(type)) {
      return next(new AppError("Valid type is required", 400));
    }
    ...
    const transaction = await GroupConsultationTransaction.create({
      consultationPackage,
      user: userId,
    ...
    const populatedTransaction = await GroupConsultationTransaction.findById(
      transaction._id
    )
...
exports.getAllGroupConsultationTransactions = catchAsync(
  async (req, res, next) => {
    const {
      status,
      startDate,
    ...
    const transactions = await GroupConsultationTransaction.find(query)
      .populate("consultationPackage", "name duration sellPrice")
      .populate("astrologer")
      .populate("user")
      .sort({ createdAt: -1 })
...
exports.getGroupConsultationTransaction = catchAsync(async (req, res, next) => {
  ...
  const transaction = await GroupConsultationTransaction.findOne({
    _id: transactionId,
    user: userId,
  })
    .populate("consultationPackage", "name duration sellPrice")
...
exports.updateGroupConsultationTransaction = catchAsync(
  async (req, res, next) => {
    const transactionId = req.params.id;
